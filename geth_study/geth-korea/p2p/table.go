...
// 노드 디스커버리 프로토콜은 연결이 가능한 RLPX를 찾을 방법을 제공한다
// ID와 수신 대기중인 노드의 엔드포인트들의 분산 DB를 유지하기 위하여
// 카뎀리아와 유사한 프로토콜을 사용한다
// 카뎀리아 : 카뎀리아(Kademlia)는 탈중앙화 분산형 P2P 컴퓨터 네트워크를 위한 분산해시테이블(DHT)을 구축하는 프로토콜
// http://wiki.hash.kr/index.php/%EC%B9%B4%EB%8E%80%EB%A6%AC%EC%95%84
package discover
